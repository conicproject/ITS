...
// ผ่าน backend proxy แทนยิงตรงไปหา IP วงในของกล้อง (10.110.1.12)
...
function LicensePlateSearch() {
    ...
    // pagination state — ตอนนี้เป็น server-side pagination
    // violations = ข้อมูลแค่ "หน้าปัจจุบัน" (สูงสุด PAGE_SIZE แถว) ไม่ใช่ทั้งหมด 200k แถวอีกต่อไป
    ...
    // ─────────────────────────────────────────────────────────────
    // map backend → UI (เก็บข้อมูลดิบทั้งหมดไว้ด้วย เผื่อ modal/sidebar ใช้)
    // ─────────────────────────────────────────────────────────────
    const mapToViolation = (row) => ({
        ...row,
        id: row.pass_id,
        lpr: row.plate_no || "ไม่ทราบทะเบียน",
        camera: row.crossing_id ? String(row.crossing_id) : "-",
        installPoint:
        ...
        roadImage: proxied(row.target_sub_url),
        position:
            row.mobile_device_latitude && row.mobile_device_longitude
                ? [row.mobile_device_latitude, row.mobile_device_longitude]
                : [13.7563, 100.5018],
    });

    // ✅ ดึงข้อมูลเฉพาะหน้าที่ต้องแสดง (limit=PAGE_SIZE, offset ตาม page)
    // แทนที่จะดึงทั้งหมด 200k+ แถวมาแล้วค่อย slice ฝั่ง frontend เหมือนเดิม
    const fetchViolations = async (payload, page = 1) => {
        try {
            ...
            const res = await apiClient.post("/api/data_search_vehicle", finalPayload);
    ...
    // ── pagination derived values (อิงจาก totalCount ที่ backend ส่งมา) ──
    const totalPages = Math.max(1, Math.ceil(totalCount / PAGE_SIZE));
    const safePage = Math.min(currentPage, totalPages);
    const startIdx = (safePage - 1) * PAGE_SIZE;

    // ✅ ไม่ต้อง slice อีกต่อไป — violations ที่ได้จาก backend คือข้อมูลของหน้านี้อยู่แล้ว
    const pagedViolations = violations;

    // ✅ เปลี่ยนหน้า = ยิง query ใหม่ไปหา backend ด้วย offset ใหม่ (ไม่ slice ข้อมูลเดิมใน memory)
    const goToPage = (page) => {
                ...
                {searchInfo && (
                    <div className="bg-sky-500/10 border border-sky-500/20 rounded-xl p-3 mb-4">
                        <p className="text-sm text-sky-300">
                            ...
                            {searchInfo.filters.camera && ` | กล้อง: ${searchInfo.filters.camera}`}
                        </p>
                    </div>
                )}
                                    ...
                                    <table className="w-full text-sm text-left">
                                        <thead>
                                            <tr className="text-[rgba(220,234,255,.5)] text-xs uppercase border-b border-white/10">
                                                ...
                                                <th className="px-4 py-3 font-medium">ความเร็ว</th>
                                    ...
                                    <div className="flex items-center justify-between px-4 py-3 border-t border-white/10">
                                        ...
                                        <div className="flex items-center gap-1">
                                            ...
                                            {Array.from({ length: totalPages }, (_, i) => i + 1)
                                                .filter((p) => {
                                                    // แสดงเฉพาะหน้าใกล้ ๆ หน้าปัจจุบัน + หน้าแรก/หน้าสุดท้าย
                                                    return (
                                                        p === 1 ||
                                                        p === totalPages ||
