...
const DetectTruckBarrier = () => {
  ...
  const [allData, setAllData] = useState([
    { lpr: '70-1234', camera: 'CAM-LEZ-01', type: 'รถบรรทุก 10 ล้อ', time: '2025-01-24 14:25', status: 'No Green List', location: 'เขตปทุมวัน (LEZ)', detail: 'เครื่องยนต์ดีเซล (ไม่ผ่านการลงทะเบียน)', fuelType: 'Diesel', position: [13.7468, 100.5349] },
    { lpr: '71-5678', camera: 'CAM-LEZ-01', type: 'รถบรรทุก 6 ล้อ', time: '2025-01-24 14:30', status: 'Green List', location: 'เขตปทุมวัน (LEZ)', detail: 'รถยนต์ไฟฟ้า (EV) - ได้รับยกเว้น', fuelType: 'Electric (EV)', position: [13.7576, 100.5654] },
    { lpr: '15-9999', camera: 'CAM-LEZ-02', type: 'รถโดยสาร', time: '2025-01-24 14:35', status: 'No Green List', location: 'แยกราชประสงค์', detail: 'ควันดำเกินมาตรฐาน', fuelType: 'Diesel', position: [13.7443, 100.5405] },
    { lpr: '73-3456', camera: 'CAM-LEZ-01', type: 'รถบรรทุก NGV', time: '2025-01-24 15:00', status: 'Green List', location: 'เขตปทุมวัน (LEZ)', detail: 'รถใช้ก๊าซ NGV - เป็นมิตรต่อสิ่งแวดล้อม', fuelType: 'NGV', position: [13.7763, 100.5718] },
    { lpr: '74-7890', camera: 'CAM-LEZ-03', type: 'รถบรรทุก 18 ล้อ', time: '2025-01-24 15:15', status: 'No Green List', location: 'ถนนพระราม 4', detail: 'ฝ่าฝืนช่วงเวลาห้ามเดินรถ', fuelType: 'Diesel', position: [13.7314, 100.5311] },
  ]);
    ...
    <div className="flex flex-col h-screen bg-gray-50 overflow-hidden relative font-sans">
      ...
      <div className="w-full mx-auto p-4 md:p-6 flex flex-col h-full max-w-[1600px]">
        ...
        <div className={`shrink-0 transition-all duration-300 ease-in-out overflow-hidden z-30 ${showFilter ? 'max-h-[500px] opacity-100 mb-1' : 'max-h-0 opacity-0 mb-0 lg:max-h-none lg:opacity-100 lg:mb-1 lg:overflow-visible'}`}>
            <div className="p-1">
                <Filter type="barrier" onSearch={handleSearch} placeholder="ค้นหาเลขทะเบียน..." showDateRange={true} />
            </div>
        </div>
        ...
        <div className="flex flex-1 gap-5 md:gap-8 overflow-hidden relative z-0">
          ...
          <div className="hidden lg:block flex-none w-[400px] xl:w-[500px] 2xl:w-[600px]">
             {/* --- แก้ไข: ใช้ selectedViolation || truckViolations[0] --- */}
            <MapSidebar data={getMapData(selectedViolation || truckViolations[0])} />
          </div>
        </div>
      </div>
    </div>
