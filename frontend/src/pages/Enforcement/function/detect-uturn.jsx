...
// ✅ map รหัส violative_action → ข้อความไทย
const VIOLATION_LABEL = {
  "1625": "ไม่สวมหมวกนิรภัย",
  "1626": "ขับรถย้อนศร",
  "1627": "ขับรถทับเส้นจราจร",
  "1628": "หยุดรถในเขตห้ามหยุด",
};

// ✅ helper สร้าง default date range = วันนี้
const getTodayRange = () => {
...
const DetectUturn = () => {
  ...
  const fetchAlarms = async (filters = {}) => {
    ...
    try {
      ...
      // ✅ ถ้าไม่มี startDate/endDate ใช้วันนี้เป็น default
  ...
  // โหลดครั้งแรก — ✅ ส่ง default range วันนี้ไปด้วย
  ...
  const handleSearch = (filters) => fetchAlarms(filters);
  ...
  return (
    <div className="w-full h-screen relative font-sans overflow-y-auto overflow-x-hidden pb-10">
      ...
      <div className="w-full mx-auto p-4 md:p-6 max-w-[1600px]">
        ...
        <div className={`transition-all duration-300 overflow-hidden ${showFilter ? "max-h-[500px] opacity-100 mb-2" : "max-h-0 opacity-0 mb-0 lg:max-h-none lg:opacity-100 lg:mb-4 lg:overflow-visible"}`}>
          <Filter type="violation" onSearch={handleSearch} />
        </div>
